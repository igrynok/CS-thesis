...
\begin{description}
	\item[Chapter \ref{cha:introduction}] In chapter \ref{cha:introduction} we give a high overview of the architecture of a typical web search engine. We describe the main steps to construct an inverted index.
	\item[Chapter \ref{cha:related-work}] In chapter \ref{cha:related-work} we survey the research related to citations in scientific publications. We overview two popular academic search engines: Google Scholar and CiteSeer.
	\item[Chapter \ref{cha:CS}] In chapter \ref{cha:CS} we describe the design of Citation Search Engine. We first show overall architecture of the proposed system and then show details of implementation of each component. 
	\item[Chapter \ref{cha:experiments}] In chapter \ref{cha:experiments} we describe user evaluation experiments and analyze the results.
	\item[Chapter \ref{cha:conclusion}] In chapter \ref{cha:conclusion} we conclude the work and describe potential future work. 
\end{description}
...
\brs{so, how did you address these challenges?}
...
\ho{Call this chapter just ``Evaluation"}
To measure \ins{the} effectiveness of Citation Search Engine we conducted evaluation experiments comparing it with other search engines.  We had two main candidates to compare with Citation Search Engine: CiteSeerX and Google Scholar. Preparatory tests showed that CiteSeerX is too slow in showing results. Moreover, users complained that resulting documents are not relevant. Too many results were from \ins{a }different \ins{domain }than Computer Science\del{ domain}, \chg{for example,}{like} Biology or Physics.  Thus\ins{,} all experiments were conducted with Google Scholar and Citation Search Engine. 
...
\ho{I guess you were too short on explaining the choices. You need to dedicate a section just to explain the trial runs of the experiments and their results. What were the other tools? why did you choose them? what is the outcome of the trial runs? Why did you decide what you decided? ...}

\section{Experiment Setup}
\ho{Usually, the experimental setup is composed of subjects, objects, process, and tools. It would be better if you can have those as subsections as follows: Data and Tools, Participants, Process. In each subsection you describe the characteristics of each.}
For evaluation experiments we used dataset of scientific articles collected by SCG members over decades. Collection contains for about 16000 scientific articles and covers various topics in Software Engineering and Programming Languages.  Dataset of Google Scholar is very large compared to our dataset, so we reduced searching space to the domain of Software Engineering and Programming Languages.  Besides we intentionally looked for experts in these domains that can participate in experiments. Nine experts with different experiences (7 PhD candidates, 1 postdoctoral researcher, 1 professor) \del{voluntarily agreed to} participate\ins{d} in \ins{the} experiments. Participants were split into two groups. Both groups were asked to perform the same tasks\chg{ h}{. H}owever the second group was asked to do one more additional task.  Time given to complete each task was limited to 5 minutes. All tasks will be described below. Each experiment was setup to last for approximately 45 minutes.  During the experiment all participants were provided with a laptop and their actions was recorded with a screen casting application.

\paragraph{Task1} \ho{use present tense to describe the experiments and tasks} As a first task\ins{,} an expert was asked to find a reference to a citation from one of his articles written in the past using the particular search engine.  Figure\ho{???} illustrates one \del{of a }citation from \chg{the}{a} paper given to an expert. An expert can read the cited sentence as well as the context of this sentence \chg{however}{but} he is not aware of referred source paper. The task is to find a referred paper that proves \ins{the }given citation.
...
During the execution of tasks \chg{following observations were recorded:}{, we recorded the following:}
\begin{itemize}[noitemsep]
	\item \del{Citation and search engine name}
	\item Search time
...
\ho{the explanation of the task is so convoluted. Please try to describe the task as simple algorithmic steps. So you do like this: First we looked for a paper published by the test subject. Second we extracted four citations form that paper ....and so on.}

\paragraph{Task1\chg{'}{$^\prime$}} This task was given to the second group as an extra task \ho{why? and why not also to the first group? How many in the first group and how many in the second group? Put these in the experimental setup section}. An expert was also given a citation to find a reference to \del{it} as in Task 1\chg{. B}{, b}ut this time a search engine was not specified and a citation was taken from \chg{the}{a} paper not authored by expert. \ho{You use the word `expert' quite a lot. Try to use the word `subject' or `participant' instead}
...
During Task1\chg{’}{$^\prime$, } we observed which search engine was used to find a reference.

\paragraph{Task2} For this task we asked every expert in advance to provide \chg{with her favorite}{a} paper that she thinks is important in her research field. We then built two automatic summaries of this paper. One summary was built using \chg{state of the art}{a state-of-the-art algorithm called} TextRank \del{algorithm} \cite{mihalcea2004textrank} \ho{how can you say state of the art and the book is published in 2004??}. \ins{The} TextRank algorithm is a graph-based ranking algorithm for Natural Language Processing (NLP) \cite{mihalcea2004graph}. It extracts sentences from the text based on their importance. We used Python implementation of this algorithm that can be found on GitHub \ho{provide a reference or a url as a footnote}.  Another summary was built using citations to the paper collected by Citation Search. \chg{If}{While} TextRank provides \chg{with the}{an} extractive summary, Citation Search collects external opinions about the paper. We limited the size of summaries to the abstract size that is approximately 9-10 sentences. 
...
\ho{I got to say at this point that the explanation of the experimental setup is not optimal. You have to organise your thoughts a little bit better and think of the reader as a complete ignorant person who needs careful taking by the hand. It seems to me that you sat down and started writing whatever jumps to your mind regarding the experimental setup. It is better if you organise your ideas and structure them on a sheet of paper first. Then, you write the skeleton of the section (subsections and sub-subsections), then start filling the space with explanations. I am just guessing that I will be saying the same about the later sections. Let's see ;)}
\section{Questionnaires before and after each experiment \ho{They are called Pre-experiment Questionnaire and Post-experiment Questionnaire}}
...
\begin{itemize}[noitemsep]
	...
	\item How often do you use \ins{the }listed \del{above} search engines? (Possible answers: everyday, a few times per week, once a week or less)
	\item Have you ever used \ins{the} following search engines?(Google Scholar, Citation Search )
\end{itemize} 
After filling the form each expert was introduced to the simple query syntax and to interfaces of search engines \ho{what search engines}. Then experiment begins with Task1 and Task1\chg{’}{$^\prime$}. After Task1 and Task1\chg{’}{$^\prime$, } \ins{the} experts were asked some general questions about their impressions on using Google Scholar and Citation Search, for example:
\begin{itemize}[noitemsep]
	\item What did you like\chg{, what you did not like}{/dislike} about using \ins{each} particular search engine?
	\item What difficulties did you have?
\end{itemize}
Right after the experiment \ins{the} experts were asked to fill a form with following questions:\ho{Why is the list of questions split?}
\begin{itemize}[noitemsep]
...
\chg{First survey, taken by experts before the experiment,}{The post-experiment questionnaire} showed that all experts use Google Scholar to find scientific literature. Some experts mentioned that they use IEEE Explorer, ACM digital library and DBLP as well. Half of the respondents use Google Scholar every day (Figure \ref{fig:q1_1_2}).\ho{Honestly, I don't think you need diagrams to describe 9 data points and definitely not a piechart. I think you should describe these results in the form of tables. However, if you really want to use diagrams, use different colors for different things.}
...
\hfill
\caption{Results for questions 1 and 2 \ho{this is not a valid caption. A figure should be self-contained and -explained. You don't want you readers to go look for the questions 1 and 2 and get back to the diagram and ... Also you have a typo in the title of the diagram. Also either put all the text inside the piechart or all the results outside. Be consistent.}}
\label{fig:q1_1_2}
\end{figure}

Four respondents answered positively on the question if they have ever used Citation Search(Figure \ref{fig:q1_3}). However all of them mentioned that they used Citation Search only a few times or rare. \ho{why isn't that result in the diagram???} 

  \begin{figure}[htp]
    \centering
    \includegraphics[scale=0.8, trim = 0mm 0mm 0mm 0mm,]{q1_3}
    \caption{results for question 3 \ho{the same}}
    \label{fig:q1_3}
  \end{figure}

\subsection{Results for Task 1}  
Table \ref{tab:search-time} and Figure \ref{fig:search-time} give results for search time \ho{What search time?} in Task 1. Table \ref{tab:search-time} shows \ins{the }mean and standard deviation values for search time for both search engines. Figure \ref{fig:search-time} illustrates a normal distribution of search time for both search engines \ho{how many data points are there anyway?}. From Table \ref{tab:search-time} and Figure \ref{fig:search-time} we observed that \ins{the} average time to find a reference for a given citation is approximately 2.5 minutes. Experts were slightly faster with finding results using Citation Search. However\ins{,} there is no statistically significant difference between search time\del{s} of Citation Search and Google Scholar according to t-test with significan\chg{t}{ce} level p = 5\%.  

  \begin{figure}[htp]
    \centering
    \includegraphics[scale=0.6, trim = 0mm 80mm 0mm 60mm,]{search-time}
    \caption{Normal distribution fit for a search time for both search engines. \ho{What is this diagram? I don't understand the x- and y-axis. How did you draw it? It seems like it is inferred, isn't it?}}
...
\ho{Tables 4.1, 4.2, 4.3 should be merged in one table.}
...
We also noted that the way search engines present results is an important factor of the search engine usability. For example, most of \ins{the} experts admitted that they like that Citation Search engine shows the exact place from the article where match was found. In contrast, Google Scholar shows a title of the article and a beginning of the abstract, so it is not clear where \ins{the} match was found. In this case experts had to open the article and make a manual search over the text.  

\subsection{Results for Task 1$^\prime$}
Results for Task 1\chg{'}{$^\prime$} are shown on Table \ref{tab:se-any}. Task 1' was given to four experts. Table \ref{tab:se-any} shows in what search engine a referred paper was found and if an expert tried to use both search engines. From Table \ref{tab:se-any} we concluded that all experts found a referred paper using Citation Search. Meantime three of experts tried both search engines and one expert did not use Google Scholar at all. 
...
\begin{tabular}{ | c | c | c | } 
 ...
 \hline
\end{tabular}
\caption{Results for Task 1'. \ins{The} table shows in what search engine a result was retrieved and if an expert tried to used both search engines during the task.}
\label{tab:se-any}
\end{center}
\end{table}  
...
Results for Task 2 are shown in Figure \ref{fig:summaries}. Figure \ref{fig:summaries} illustrates scores from 0 to 10 given by expert to summaries generated with TextRank and citation from Citation Search.
  \begin{figure}[htp]
    ...
    \caption{Scores from 0 to 10 given by experts in Task 2 to \ins{the} summaries generated with TextRank and citations from Citation Search. \ho{What is the y-axis? I guess it is the expert number. You should make it clear. Also where are the other two?}}
    \label{fig:summaries}
  \end{figure}
Almost all experts except one gave better score to the summary composed by citations from Citation Search. According to \chg{ttest}{t-test} with a significance level p = 5\% there is a significant difference between scores given to summaries generated with TextRank and Citation Search. Experts noted that a summary generated with TextRank \ugh{tends to consist of too general or not important for understanding a paper sentences}. According to experts, \ins{the }summary composed with citations tends to contain sentences more relevant for understanding \chg{a}{the summarized} paper. However, \ugh{sometimes sentences rephrase each other not adding new meaning to the summary}. Also, compared to TextRank there is no \del{a} natural flow in \chg{a}{the} summary from citations, \ugh{saying in other words sentences in a summary are not ordered in a story manner}.       

\subsection{Final Questionnaire}
Final questionnaire show that \ugh{some}\ho{how many??} experts were \ugh{pleasantly surprised by the capabilities of another type of search engine}. All experts answered positively \ins{when they were asked} \chg{if}{whether} they are willing to continue to use Citation Search. Some experts specified that they will use Google Scholar and Citation Search for different purposes. According to some\ho{how many??} experts Citation search is more appropriate to search for a related work on the given topic. Others\ho{how many} think that Citation Search is good to prove claims while writing a scientific paper. One expert opinion states that Citation Search is useful for discovering new works in the given domain. Experts \ugh{highlighted following feature of Citation Search}: possibility to see citations with a context, possibility to search by bibliography entries.  

\subsection{Results Summary}
In our evaluation experiments we compared Citation Search with Google Scholar. During \ins{the }experiments we collected statistics on search time, number of queries and average number of words in queries. The results show that Citation Search performs slightly better for mean value of search time, \chg{however}{but} there is no statistically significant difference among search engines. Overall, given that Google Scholar is one of the most popular academic search engines in WWW, Citation Search might be a good alternative to Google Scholar \ho{what a strong claim?? Lighten it a little bit}. Indeed, when experts have a possibility to choose between two search engines, all experts succeeded in the task accomplishment using Citation Search.
