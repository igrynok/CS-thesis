...
\begin{tabular}{  c | c | c | c } 
 ...
 \hline  
\end{tabular}
\caption{The table describes experts participated in the experiments, their domain of interests and academic experience in years.}
\label{tab:experts}
\end{center}
\end{table}
\subsection{Process}
Participants were split into two groups. All experiments were conducted in two days and each day was dedicated to one group. Both groups were asked to perform the same tasks. However the second group was asked to do one more additional task (see Table \ref{tab:groups}). The idea of giving an additional task to the second group came after conducting experiments with the first group on the first day. Time given to complete each task was limited to 5 minutes. All tasks will be described below.  
\begin{table}[ht]
\begin{center}
\begin{tabular}{  c | c | c  } 
 \hline
 ...
 Group2 & P2, P7, P8, P9 & Task1, Task1{$^\prime$}, Task2\\  
 \hline  
\end{tabular}
...
\begin{enumerate}
	...
	\item We delete extracted citations from Citation Search so the test subject could not find an exact match using Citation Search.
\end{enumerate}
During the experiment.
\begin{enumerate}
	\item We let the test subject read one cited sentence as well as the context of this sentence.
	\item We ask the test subject to find a referred paper that proves the given claim using the given search engine (Citation Search or Google Scholar).
	\item We repeat steps for four citations every time changing the using search engine. 
\end{enumerate}
...
\begin{itemize}[noitemsep]
	...
	\item \emph{Number of words in each query}, numbers of words in each query made by the test subject while search.
	\item \emph{Expert comments}, any comments made by the test subject during the task execution.
\end{itemize}  

\paragraph{Task1{$^\prime$}} Task1{$^\prime$} was given to the second group as an extra task. By conducting this task, we would like to know which search engine would the test subject use if it is not specified in the task description. As in Task1 a test subject was also given a citation to find a reference to, but this time a search engine was not specified and a citation was taken from a paper not authored by test subject. Every participant receive only one citation for this task. As for the previous task the citation was removed from Citation Search before the experiment. 

During Task1{$^\prime$,} we observed which search engine was used to find a reference. 

\paragraph{Task2} In Task2 we asked participants to compare two summaries generated with Citation Search and TextRank algorithm \cite{mihalcea2004textrank}. The TextRank algorithm is a graph-based ranking algorithm for Natural Language Processing (NLP) \cite{mihalcea2004graph}. It extracts sentences from the text based on their importance. We used Python implementation of this algorithm that can be found on GitHub \footnote{\url{https://github.com/davidadamojr/TextRank}}.We use following procedure to conduct Task1:
\\Before the experiment.
\begin{enumerate}
	\item We asked every participant in advance to provide a paper that she thinks is important in her research field.
	\item We built two summaries: . We limited the size of summaries to the size of an abstract of a paper, which is approximately 9-10 sentences. 
\end{enumerate}
During the experiment.
\begin{enumerate}
...
To conduct Task2 we asked every expert in advance to provide a paper that she thinks is important in her research field. We then built two automatic summaries of that paper. One summary was built using TextRank \cite{mihalcea2004textrank}. The TextRank algorithm is a graph-based ranking algorithm for Natural Language Processing (NLP) \cite{mihalcea2004graph}. It extracts sentences from the text based on their importance. We used Python implementation of this algorithm that can be found on GitHub \footnote{\url{https://github.com/davidadamojr/TextRank}}.  Another summary was built using citations to the paper collected by Citation Search. While TextRank provides an extractive summary, Citation Search collects external opinions about the paper. We limited the size of summaries to the abstract size that is approximately 9-10 sentences. 
...
\section{Pre-experiment Questionnaire and Post-experiment Questionnaire}
