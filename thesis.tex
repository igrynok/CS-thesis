...
Searching large collections of documents manually is not time efficient. The common way of making search effective in terms of time is \ins{automated} indexing \ins{of} documents in advance. Before \ins{the} indexing procedure, documents should be parsed and \ugh{analyzed}\brs{what to you mean by this? analyzed how? if I am indexing a pure text file there is not much analysis to be done right?}. In this work we propose a search system over collections of scientific articles in PDF format. Scientific articles are different from ordinary documents in following sense\chg{.}{:} First, scientific articles in PDF format need to be converted into textual representation\chg{.}{;} Second, scientific articles have a certain layout format, \ugh{where each section needs to be considered separately}\brs{this is not important. we don't focus on which sections does what}. In \ugh{Citation Search Engine}\brs{what is a Citation Search Engine? define before use: we call our tool bla bla bla}, we do not aim to index entire contents of articles. Instead, we aim to take advantage of indexing extracted meta-information. We argue that considering meta-information enhances a search system. \brs{What meta information? be more precise, say that we mean citations and use it as a segue way into the next paragraph about citations}

In general, a citation is composed of two parts: a citation text in a body of a document and a bibliographic link in a reference section. Citation Search Engine indexes both parts separately \ugh{thus allowing \ins{the} user to make a choice between different indexed collections}\brs{wat? not sure what this means}. We also store a context of a citation, i.e., text surrounding a citation, to provide better understanding of a cited text. Bibliographic links identify the cited origin paper. By finding bibliographic links pointing to the same original source we can merge citations related to the same paper. Such functionality is especially useful to automatically find citations of a given article in other articles.   

\brs{I expected a high level overview of what was done and I did not get it. You focus on many unimportant details here (\ie saving the context of the citation) and you don't explain more important details (\ie How does the search work? what do the citations have to do with it, what meta data, \etc)}

\section{Contributions}
Following are the main contributions of this work:
\begin{itemize}
	\item A novel IR\brs{abbreviation only mentioned in the abstract, abstract is not a part of the text} system for scientific articles based on citations,
...
\begin{description}
	\item[Chapter \ref{cha:introduction}] \ugh{gives}\brs{Chapters don't do these thing, authors do. rephrase to 'in chapter bla We give bla'} a high overview of the architecture of a typical web search engine. It describes the main steps to construct an inverted index.
...
The workflow of the system is shown on Figure \ref{fig:cs-activity-diagram}. The first operation performed by the system is parsing. The \emph{Parser} converts a PDF file into the text. Then it extracts meta information, like citations and references, from the textual representation of the file. Next it packages extracted information into data units corresponding to formats acceptable by \emph{Indexer} and \emph{Meta data storage}. A data unit publishable to the \emph{Indexer} consist of a citation, that should be indexed and an additional information related to this citation (citation context, a file URI, bibliographic references) that should be stored. A data unit publishable to the \emph{Meta data storage} consist of a citation, a source paper identifier and bibliographic references. We use \emph{Meta data storage} for aggregating citations referred to the same source. Once the \emph{Parser} processed the PDF file it can proceed to the next paper if there is any left. When all papers are processed, user can make queries over \emph{Search Web App}.
  ...
  \begin{figure}[htp]
    ...
    \caption{Activity diagram of Citation Search.\brs{first action is 'Parsing A PDF file' note the article}}
...
\begin{tabular}{ | l | p{7cm} | } 
 ...
 \hline
\end{tabular}
\caption{Frequent patterns in using square brackets ([ and ])for citing \brs{more space between text examples}}
\label{tab:citations}
\end{center}
\end{table}
...
As usual we try to recognize common patterns covering the majority of bibliographic links. Table \ref{tab:bibl.links} shows some examples of bibliographic links. First,we noticed that if a bibliographic link contains some sort of \ugh{quotes}\brs{Quotation marks}, for example, double quotes (``'') or single quotes (`'), then it is highly probable that a title is enclosed by these quotes. Then, we made some observations for bibliographic links without quotes. Very often, a bibliographic link is structured as follows: it begins by listing the paper's authors, then the title, and then comes the rest of the link (see Figure \ref{fig:link}). It turned out that Core NLP library used for breaking text into sentences is good in dividing a link into parts according to our view. In most of cases it is enough to take the second part of the bibliographic link to be a title. \brs{unclear how Core NLP is used here}
...
There are two systems where documents are published to: Solr and MongoDB. Solr is used for indexing citations and MongoDB for storing relations in meta-data. We aim to use MongoDB for aggregating citations referred to the same source paper. \brs{Tie in to the story of figure 3.1 a bit more strongly, say that mongo is the Meta Data Storage module, and that Solr is the indexer module}

\paragraph{Publishing documents to Solr}
The common way to interact with Solr is using REST API \brs{Link or ref to what a REST API is}. Solr provides client libraries for many programming languages to handle interactions with Solr's REST API. In our project we used SolrJ client library for Java language. This library abstract interaction with Solr into java objects instead of using typical XML request/response format. Basic Solr storage unit is called document and SolrJ has document abstraction implementation. For every detected citation we compose a document to publish. Figure \ref{fig:doc} represents a structure of documents we publish to Solr.
...
As a meta information storage we used MongoDB. MongoDB is a NoSQL document-oriented database. Data in MongoDB is stored in JSON-like documents with a dynamic schema. The format of stored data is called BSON, which stands for Binary JSON. BSON is an open standard developed for human readable data exchange \footnote{BSON specification, \url{http://bsonspec.org/}}. MongoDB requires a little amount of configuration to start to work with (see Appendix \ref{app:mongo}). 

\brs{There seems to be some duplication between the publishing documents part and the indexer/meta data storage part. I don't know what is what and why they are separate sections. The flow is a bit confusing}
