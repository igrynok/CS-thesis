...
The increasing amount of research literature produced by the scientific community poses a number of challenges to the task of finding relevant related work to newly written papers.
This fact leads to an overall decrease of quality of research papers.
...
Current solutions to the problem, such a Google Scholar, are typically based on a keyword search, and thus do not work well for the case of finding relevant citations.
This happens due to the following facts that they do not take document structure into account and they search for claims in the parts of documents that do not contain any claims at all, and therefore return a large amount of irrelevant results.
...
\paragraph{Task2} In Task2 we asked participants to compare two summaries generated with Citation Search and TextRank\cite{mihalcea2004textrank} algorithms. The TextRank algorithm is a graph-based ranking algorithm for Natural Language Processing (NLP) \cite{mihalcea2004graph}. It extracts sentences from the text based on their importance. We use following procedure to conduct Task2:
\\Before the experiment.
\begin{enumerate}
	\item We ask every participant in advance to provide a paper that she thinks is important in her research field. 
	\item We verify that a provided paper was cited at least by ten other papers in the Citation Search dataset.
	\item We build a first summary using the TextRank algorithm. We use Python implementation of this algorithm that can be found on GitHub \footnote{\url{https://github.com/adamfabish/Reduction}}. We extract the text of a paper and feed it to TextRank. We limit the size of summaries to the size of an abstract in a paper, that is approximately 9-10 sentences.
	\item We build a second summary using citations to the paper collected by Citation Search. Citation Search might collect more than ten citations of a paper. In this case we pick ten sentences randomly. Again we limit the size of summaries to the size of an abstract in a paper. 
\end{enumerate}
During the experiment.
\begin{enumerate}
...
In Task1 we measured search time. Each participant perofrmed Task1 4 times: 2 times with Citation Search and 2 times with Google Schoalr. In overall, we made 18 measurements for Citation Search and 18 measurements for google Schoalr. Figure \ref{fig:boxplot-search-time} illustrates results for search time in Task 1 using boxplots \footnote{\url{http://en.wikipedia.org/wiki/Box_plot}}. Table \ref{tab:search-time} shows the mean and standard deviation of search times for both search engines. From Table \ref{tab:search-time} we observed that the average time to find a reference for a given citation is approximately 2.5 minutes. Participants were slightly faster with finding results using Citation Search. However, there is no statistically significant difference between search times of Citation Search and Google Scholar according to t-test with significance level p = 5\%. 

  \begin{figure}[htp]
    \centering
    \includegraphics[scale=0.6, trim = 0mm 60mm 0mm 50mm,]{boxplot-search-time}
    \caption{Boxplot of search times in Task1 for both search engines. The red band inside a box is a second quartile (the median). The bottom and top of the box are the first and third quartiles. The end of whiskers represent the minimum and maximum values for search times.}
...
\begin{tabular}{ | c | c | c | } 
 ...
 \hline
\end{tabular}
\caption{The mean and standard deviation of search times for Citation Search and Google Scholar in Task 1.}
...
\end{table}
 ...
 During the experiments we noticed that participants were more familiar with Google Scholar search interface so participants spent some time exploring Citation Search interface. This could affect search time for Citation Search making it longer.

We also noted that the way search engines present results is an important factor of the search engine usability. For example, most of the participants admitted that they like that Citation Search engine shows the exact place from the article where match was found. In contrast, Google Scholar shows a title of the article and a beginning of the abstract, so it is not clear where the match was found. In this case participants had to open the article and make a manual search over the text.  

\subsection{Results for Task 1$^\prime$}
Results for Task 1{$^\prime$} are shown on Table \ref{tab:se-any}. Task1' was given to four participants. Table \ref{tab:se-any} shows in what search engine a referred paper was found and if a participant tried to use both search engines. From Table \ref{tab:se-any} we concluded that all participants found a referred paper using Citation Search. Meantime three of participants tried both search engines and one participant did not use Google Scholar at all. 
...
\begin{tabular}{ c | c | c } 
 ...
 P7 & Citation Search & Yes\\ \hline
 P9 & Citation Search & Yes\\ \hline
 P4 & Citation Search & Yes\\ \hline
 P2 & Citation Search & No\\   
 \hline
\end{tabular}
\caption{Results for Task 1$^\prime$. The table shows in what search engine a result was retrieved and if a participant tried to used both search engines during the task.}
\label{tab:se-any}
\end{center}
\end{table}  
...
-show title of a paper with the citaion
\subsection{Feature extensions}
-use to generate summaries
-craw paper from open sources in Web 
-automatic citaion aggregation for any paper in Web
